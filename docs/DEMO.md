...
| 2 | Where does the baseline perception model fail? | Guided tour + Failure Explorer | tour step 2, **One missed pedestrian** (the hero frame, per-model claims), and Failure Explorer's **Detail** — now the first block on the page, auto-selected, with the **Model** radio and the GT / Predictions / Overlay toggle beside the image | local: 2026-08-22 · live: pending |
