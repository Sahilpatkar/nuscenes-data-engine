 ...
 * that happens to sit nearby — it is the panel the chart is drawn inside. Eyebrow,
 * clauses-as-chips and chart share one bordered surface, and the control clause
 * (the arm every other bar is measured against) is the one chip in accent. The
 * chart cannot be read without reading what was held constant.
 *
 * Beats: the four cards → the night-share line that says what changed → the fair
 * comparison panel (chips, chart, the charted arms' raw ids) → the winner, which
 * is the sentence the section lands on.
 *
 * No tile is accented here. The section's one point of emphasis is the highlighted
 * bar and the winner sentence that names it; a fifth accent in the card row would
  ...
  return (
    <>
      ...
      <div className="fairness">
        ...
        <p className="mono muted charted-ids">
          <Ticks text={data.charted_ids_caption} />
        </p>
      </div>
 ...
 * so the site restores the same emphasis with its own type. Nothing is reworded
