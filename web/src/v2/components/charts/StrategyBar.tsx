 ...
 * `aria-labelledby` still points at that title, so its accessible name is what it
 * was. Only the paint changes: the report's chart slots, Source Code Pro labels.
 *
 ...
 * highlighted one, which is `--chart-accent`. Bronze and red are neighbours under
 ...
 * which arm lost night mAP. The value labels are `--ink`, never a mark colour,
 * which is also what keeps the dark mode's `--chart-neg` off of type.
 *
...
/**
 * Source Code Pro at the 10 px `.chart-step` size advances ≈ 6 px per character
 * (its 600/1000 em advance — the same figure IBM Plex Mono gives at this size,
 * which is why the wrap points did not move when the family did).
 */
...
export function StrategyBar({ chart }: { chart: StrategyChart }): JSX.Element | null {
  ...
  return (
    <div className="chart strategy-chart">
      ...
      <div className="strategy-plot">
        {/* No `role="img"` on the svg: its bands are focusable, and a focusable
            ...
            to ignore and a keyboard still lands on. The title names it. */}
        <svg
          className="chart-svg"
